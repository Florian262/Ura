...
describe('B1 Chapter 8 Writing Validation (Chapter ID 28)', () => {
  it('passes valid guided paragraph using relative participle and reflexive kendi', () => {
    // 135 characters (>= 60)
    const res = evaluateWriting(24, "Benim yazdığım otobiyografi çok ilginçti çünkü kendi hayatımı ve başarılarımı anlattım. Eğer çalışırsam Türkçeyi öğreneceğim.");
  ...
  it('passes valid guided paragraph using relative participle and conditional form', () => {
    // 101 characters (>= 60)
    const res = evaluateWriting(24, "Türkçe öğrenmek için başladığım bu yolda her gün çalışırsam hedeflerimi kolayca başarabilirim.");
    expect(res.status).toBe('success');
    expect(res.feedback).toContain('Keni shkruar një autobiografi ose përmbledhje të saktë');
  });

  it('flags short inputs below 60 characters', () => {
    const res = evaluateWriting(24, "Yazdığım otobiyografi."); // < 60 chars
  ...
  it('flags missing relative participles', () => {
    ...
    const res = evaluateWriting(24, "Ben kendi hayatımı yazdım ve bu kursta çok çalıştım. Gelecekte başarılı olmak istiyorum.");
    expect(res.status).toBe('error');
  ...
  it('flags missing passive/reflexive/conditional forms', () => {
    ...
    const res = evaluateWriting(24, "Bizim gördüğümüz ve öğrendiğimiz konular çok kolaydı. Yarın yeni bir kitaba başlayacağız.");
    expect(res.status).toBe('error');
