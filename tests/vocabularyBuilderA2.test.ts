...
import { a2VocabularyData } from '../src/infrastructure/db/a2Vocabulary';

describe('A2 Thematic Vocabulary Data Tests', () => {
  it('should contain exactly 500 vocabulary items', () => {
    expect(a2VocabularyData.length).toBe(500);
  });

  it('should have exactly 10 unique categories', () => {
    const categories = new Set(a2VocabularyData.map(item => item.category));
  ...
  it('should contain exactly 50 words in each of the 10 categories', () => {
    const categoryCounts: Record<string, number> = {};
    
    a2VocabularyData.forEach(item => {
    ...
    expectedCategories.forEach(cat => {
      expect(categoryCounts[cat]).toBe(50);
    });
  });
  ...
  it('should have zero word duplications between A1 and A2 databases', () => {
    const a1Words = new Set(a1VocabularyData.map(item => item.word.toLowerCase().trim()));
    a2VocabularyData.forEach(item => {
      const a2Word = item.word.toLowerCase().trim();
      expect(a1Words.has(a2Word), `Duplicate word found in A1 and A2 databases: "${item.word}"`).toBe(false);
    });
  });

  it('should conform to the A2VocabularyItem schema constraints', () => {
    a2VocabularyData.forEach(item => {
