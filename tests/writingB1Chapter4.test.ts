...
describe('B1 Chapter 4 Writing Validation (Chapter ID 24)', () => {
  it('passes valid guided paragraph using necessity (-meli/-malı)', () => {
    const res = evaluateWriting(24, "Yeni evime geçen hafta taşındım. Evde çok gürültü yapmamalıyız çünkü komşular rahatsız olabilir.");
  ...
  it('passes valid guided paragraph using temporal since (-diğinden beri)', () => {
    const res = evaluateWriting(24, "Türkçe öğrenmeye başladığımdan beri her gün yeni kelimeler çalışıyorum. Daha çok pratik yapmalıyım.");
    expect(res.status).toBe('success');
    expect(res.feedback).toContain('Keni shkruar një paragraf të saktë');
  });

  it('flags short inputs below 60 characters', () => {
    const res = evaluateWriting(24, "Çok çalışmalıyım."); // < 60 chars
    expect(res.status).toBe('error');
    expect(res.feedback).toContain('të paktën 60 karaktere');
  });

  it('flags missing required grammar structures', () => {
    const res = evaluateWriting(24, "Yeni bir ev kiraladım. Komşularımla tanıştım ve apartman kurallarını hemen öğrendim. Her şey harika."); // >= 60 chars but no -meli or -diğinden beri
    expect(res.status).toBe('error');
  ...
  it('flags vowel harmony violations in temporal since suffix', () => {
    ...
    const res = evaluateWriting(24, "Buraya geldiğından beri her gün yeni kelimeler çalışıyorum ve derslerimi tekrar ediyorum.");
    expect(res.status).toBe('error');
  ...
  it('flags consonant mutation violations in temporal since suffix', () => {
    ...
    const res = evaluateWriting(24, "Temizlik yapdığımdan beri ev tertemiz duruyor. Herkes çok dikkatli davranmalı.");
    expect(res.status).toBe('error');
