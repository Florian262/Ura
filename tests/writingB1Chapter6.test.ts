...
describe('B1 Chapter 6 Writing Validation (Chapter ID 26)', () => {
  it('passes valid guided paragraph using two passive verbs', () => {
    // 101 characters (>= 60)
    const res = evaluateWriting(26, "Geçen yıl harika bir film çekildi ve televizyonda yayınlandı. Herkes bu yapımı büyük bir ilgiyle izledi.");
  ...
  it('passes valid guided paragraph using other passive verbs', () => {
    // 98 characters (>= 60)
    const res = evaluateWriting(26, "Bu roman ünlü yazar tarafından yazıldı. Kitap çok izlenen bir film projesi haline getirildi.");
    expect(res.status).toBe('success');
    expect(res.feedback).toContain('Keni shkruar një paragraf të saktë');
  });

  it('flags short inputs below 60 characters', () => {
    const res = evaluateWriting(26, "Film çekildi."); // < 60 chars
  ...
  it('flags missing passive verbs (zero passive verbs)', () => {
    ...
    const res = evaluateWriting(26, "Ben dün sinemaya gittim ve güzel bir film izledim. Yeni romanı hemen satın aldım.");
    expect(res.status).toBe('error');
  ...
  it('flags inputs with only one passive verb', () => {
    ...
    const res = evaluateWriting(26, "Bu roman ünlü bir yazar tarafından yazıldı. Ben kitabı dün kütüphanede okumaya başladım.");
    expect(res.status).toBe('error');
    expect(res.feedback).toContain('Duhet të përdorni të paktën dy folje në formën pasive');
  });
});
